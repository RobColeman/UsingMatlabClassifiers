function [X Y Xtr Xte Ytr Yte rnd data r w d]=fhalfmoon(N,d,r,w,trteratio)
% Usage: data=fhalfmoon(N,d,r,w,trterati
...
if nargin<5, trteratio = 0.75;end
if nargin<4, w=6;end
...
Y = data(:,3)+1;
data = [X Y];
Xtr = X(rnd(1:round(N*trteratio)),:);
Xte = X(rnd(round(N*trteratio)+1:end),:);
Ytr = Y(rnd(1:round(N*trteratio)));
Yte = Y(rnd(round(N*trteratio)+1:end));



save halfmoon X Y Xtr Xte Ytr Yte N rnd data r w d;
end
